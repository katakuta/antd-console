...
export default function FormDemoPage() {
  ...
  const tabItems = [
    {
      key: 'basic', label: t('formDemo.basicForm'),
      children: (
        <Card>
          <Form form={form1} layout="vertical" onFinish={() => message.success(t('formDemo.submitSuccess'))} style={{ maxWidth: 600 }}>
            <Form.Item name="name" label="Name" rules={[{ required: true }]}><Input placeholder="Enter name" /></Form.Item>
            <Form.Item name="type" label="Type" rules={[{ required: true }]}>
              <Select placeholder="Select type" options={[{ label: 'Option A', value: 'a' }, { label: 'Option B', value: 'b' }, { label: 'Option C', value: 'c' }]} />
            </Form.Item>
            <Form.Item name="date" label="Date" rules={[{ required: true }]}><DatePicker style={{ width: '100%' }} /></Form.Item>
            <Form.Item name="amount" label="Amount"><InputNumber min={0} max={999999} style={{ width: '100%' }} placeholder="Enter amount" /></Form.Item>
    ...
    {
      key: 'step', label: t('formDemo.stepForm'),
      children: (
        <Card>
          ...
          <Form form={form2} layout="vertical" onFinish={handleStepFinish} style={{ maxWidth: 600 }}>
            {currentStep === 0 && (<>
              <Form.Item name="firstName" label="First Name" rules={[{ required: true }]}><Input placeholder="First name" /></Form.Item>
              <Form.Item name="lastName" label="Last Name" rules={[{ required: true }]}><Input placeholder="Last name" /></Form.Item>
              <Form.Item name="email" label="Email" rules={[{ required: true, type: 'email' }]}><Input placeholder="email@example.com" /></Form.Item>
            </>)}
            {currentStep === 1 && (<>
              <Form.Item name="username" label="Username" rules={[{ required: true }]}><Input placeholder="Choose a username" /></Form.Item>
              <Form.Item name="password" label="Password" rules={[{ required: true, min: 6 }]}><Input.Password placeholder="At least 6 characters" /></Form.Item>
              <Form.Item name="role" label="Role"><Select placeholder="Select role" options={[{ label: 'Admin', value: 'admin' }, { label: 'User', value: 'user' }]} /></Form.Item>
            </>)}
            ...
            <Space>
              ...
              <Button type="primary" htmlType="submit">{currentStep === 2 ? t('common.submit') : 'Next'}</Button>
            </Space>
          </Form>
        </Card>
      ),
    ...
    {
      key: 'dynamic', label: t('formDemo.dynamicForm'),
      children: (
        <Card>
