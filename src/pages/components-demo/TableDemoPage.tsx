...
const CATEGORY_OPTIONS = [
  { label: 'Electronics', value: 'Electronics' }, { label: 'Furniture', value: 'Furniture' },
  { label: 'Audio', value: 'Audio' }, { label: 'Storage', value: 'Storage' },
];
...
export default function TableDemoPage() {
  ...
  const handleRowAction = (actionKey: string, record: DemoRecord) => {
    if (actionKey === 'edit') { setEditingRecord(record); form.setFieldsValue(record); setModalOpen(true); }
    if (actionKey === 'delete') handleDelete(record);
  };

  const handleDelete = async (record: DemoRecord) => {
    try { await mockDeleteRecord(record.id); message.success(t('common.operationSuccess')); fetchData(); }
  ...
  const columns: DataTableColumn<DemoRecord>[] = [
    ...
    { key: 'status', title: 'Status', dataIndex: 'status', width: 100, render: (_: unknown, record: DemoRecord) => <Tag color={record.status === 'Active' ? 'green' : record.status === 'Pending' ? 'orange' : 'red'}>{record.status}</Tag> },
    { key: 'priority', title: 'Priority', dataIndex: 'priority', width: 100, render: (_: unknown, record: DemoRecord) => <Tag color={record.priority === 'High' ? 'red' : record.priority === 'Medium' ? 'blue' : undefined}>{record.priority}</Tag> },
  ...
  return (
    <Space direction="vertical" size="middle" style={{ width: '100%' }}>
      ...
      <Modal title={editingRecord ? t('tableDemo.editRecord') : t('tableDemo.createRecord')} open={modalOpen}
        onCancel={() => setModalOpen(false)} onOk={handleModalOk} confirmLoading={submitting} destroyOnClose>
        <Form form={form} layout="vertical" style={{ marginTop: 16 }}>
          <Form.Item name="name" label="Name" rules={[{ required: true, message: 'Name is required' }]}><Input placeholder="Record name" /></Form.Item>
          <Row gutter={16}>
            <Col span={12}><Form.Item name="category" label="Category" rules={[{ required: true }]}><Select options={CATEGORY_OPTIONS} placeholder="Select category" /></Form.Item></Col>
            <Col span={12}><Form.Item name="status" label="Status" rules={[{ required: true }]}><Select options={STATUS_OPTIONS} placeholder="Select status" /></Form.Item></Col>
          </Row>
          <Form.Item name="priority" label="Priority" rules={[{ required: true }]}><Select options={PRIORITY_OPTIONS} placeholder="Select priority" /></Form.Item>
          <Form.Item name="description" label="Description"><Input.TextArea rows={3} placeholder="Enter description" /></Form.Item>
        </Form>
      </Modal>
    </Space>
  );
