  ...
  const userCols = [
    {
      ...
      key: 'name',
      width: 180,
      render: (_: unknown, r: DemoUser) => (
        <Space>
          <Avatar size={24} style={{ background: A.sapphire, fontSize: 12 }}>
            {r.name.charAt(0)}
          </Avatar>
          <span>{r.name}</span>
        </Space>
  ...
  return (
    <Space direction="vertical" size="large" style={{ width: '100%' }}>
          ...
          <Col xs={24} sm={12} lg={6} key={s.title}>
            <Card style={cardBd} styles={{ body: cardBody }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                <div>
                  ...
                  <Statistic
                    ...
                    valueStyle={{ fontSize: 30, fontWeight: 700, letterSpacing: '-0.02em', lineHeight: 1, marginTop: token.marginXXS }}
                  />
                </div>
                <div
                  style={{
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: token.marginXS, marginTop: token.marginSM }}>
                ...
                <Typography.Text strong style={{ fontSize: 13, color: s.up ? A.emerald : A.rose }}>
                  {s.change}
                </Typography.Text>
            ...
            <ResponsiveContainer width="100%" height={220}>
              <LineChart data={stats?.userGrowth?.slice(-7) ?? []} margin={{ top: 4, right: 16, bottom: 0, left: 0 }}>
                ...
                <Line
                  ...
                  stroke={A.sapphire}
                  strokeWidth={2.2}
                  dot={{ r: 3, fill: A.sapphire, stroke: '#fff', strokeWidth: 2 }}
              ...
              {sysMetrics.map((m) => (
                <div key={m.label}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: token.marginXXS }}>
                    ...
                    <Typography.Text strong style={{ fontSize: 13, color: m.color }}>
                      {m.value}%
                    </Typography.Text>
                  </div>
                  <Progress
                ...
                <Col xs={12} sm={8} key={i}>
                  <button
                    ...
                    style={{
                      ...
                      borderRadius: token.borderRadiusSM,
                      padding: pad(token.paddingSM + 2, token.padding),
                      cursor: 'pointer',
                      display: 'flex',
                      alignItems: 'center',
                      gap: token.marginSM,
      ...
      <Row gutter={[token.margin, token.margin]}>
        <Col xs={24} lg={14}>
          <Card
            title={<Label>{t('dashboard.recentActivity')}</Label>}
            extra={
              <Button type="link" onClick={() => navigate('/examples/users')} style={{ fontWeight: 500 }}>
